...
std::string TEST_HTML_ABSOLUTE_LINK = R"(<a href=")" + TEST_FULL_URL + R"(">Link</a>)";

TEST(HtmlParser, ParseWithRelativeUrl) {
    HtmlParser parser;
    int nLinks = 0;
...
TEST(HtmlParser, ParseWithAbsoluteUrl) {
    HtmlParser parser;
    int nLinks = 0;
    char* result = parser.Parse(
            (char*)TEST_HTML_ABSOLUTE_LINK.c_str(),
            (int)TEST_HTML_ABSOLUTE_LINK.size(),
            (char*)TEST_BASE_URL.c_str(),
            (int)TEST_BASE_URL.size(),
            &nLinks
    );
